...
describe('CodexStatusScreen', () => {
  it('lists the paths that were searched when the CLI is missing', () => {
    render(<CodexStatusScreen state={{ status: 'not-installed', searched: ['C:/one/codex.exe', 'C:/two/codex.exe'] }} />)
  ...
  it('lets the user accept a non-isolated environment for this run', () => {
    render(<CodexStatusScreen state={{ status: 'not-isolated', problems: ['CODEX_HOME condiviso'] }} />)
    ...
    expect(codexBlocking({ state: { status: 'not-isolated', problems: [] }, isolationAccepted: true })).toBe(false)
    expect(codexBlocking({ state: { status: 'crashed', message: 'boom' }, isolationAccepted: true })).toBe(true)
  })

  it('shows the crash message and retries through the store', () => {
    const retry = vi.fn()
    render(<CodexStatusScreen state={{ status: 'crashed', message: 'exit code 1' }} onRetry={retry} />)
  ...
  it('shows a spinner while the session is starting', () => {
    ...
    expect(screen.getByRole('status', { name: 'Collegamento a Codex in corso' })).toBeInTheDocument()
