...
function gameWith(p: { id: string; sans: string[]; classifications: (MoveClassification | undefined)[]; eco?: string; result?: Game['result'] }): Game {
  ...
  return {
    ...
    analysis: { accuracy: { w: 80, b: 70 }, acpl: { w: 40, b: 60 }, keyMoments: [], analyzedAt: '2026-03-01T10:40:00.000Z' },
    ...(p.eco ? { opening: { eco: p.eco, name: 'Partita spagnola', lastBookPly: 6 } } : {})
  }
}

const profileWith = (openingStats: Profile['openingStats']): Profile => ({ ...EMPTY_PROFILE, openingStats })

describe('firstDeviation', () => {
  it('finds the first inaccuracy or worse the user played inside the opening', () => {
    const game = gameWith({ id: 'g1', sans: ['e4', 'e5', 'Nf3', 'Nc6', 'Bb5', 'a6'], classifications: [undefined, undefined, 'inaccuracy', undefined, 'blunder'] })
    ...
    expect(deviation?.epd).toBe(epdOf('rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2'))
  })

  it('ignores the opponent and the moves the engine never judged', () => {
    const game = gameWith({ id: 'g1', sans: ['e4', 'e5', 'Nf3'], classifications: [undefined, 'blunder', undefined] })
    expect(firstDeviation(game)).toBeNull()
  })

  it('ignores a mistake played past the twentieth ply', () => {
    const sans = ['e4', 'e5', 'Nf3', 'Nc6', 'Bb5', 'a6', 'Ba4', 'Nf6', 'O-O', 'Be7', 'Re1', 'b5', 'Bb3', 'd6', 'c3', 'O-O', 'h3', 'Nb8', 'd4', 'Nbd7', 'c4', 'c6']
    const classifications = sans.map((_, index) => (index === 20 ? ('blunder' as MoveClassification) : undefined))
...
describe('buildOpeningsOverview', () => {
  const stats = {
    C60: { eco: 'C60', name: 'Partita spagnola', games: 4, wins: 2, draws: 1, losses: 1, avgAccuracyFirst10: 82.5 },
    B20: { eco: 'B20', name: 'Siciliana', games: 1, wins: 0, draws: 0, losses: 1, avgAccuracyFirst10: 60 }
  }
  ...
  it('groups the same deviation across games into one line with a count', () => {
    const games = [1, 2, 3].map((n) =>
      gameWith({ id: `g${n}`, sans: ['e4', 'e5', 'Nf3', 'Nc6', 'Bb5'], classifications: [undefined, undefined, 'mistake'], eco: 'C60' })
    )
  ...
  it('keeps a game without an opening out of the deviations', () => {
    const games = [gameWith({ id: 'g1', sans: ['e4', 'e5', 'Nf3'], classifications: [undefined, undefined, 'blunder'] })]
    expect(buildOpeningsOverview(profileWith(stats), games).every((row) => row.deviations.length === 0)).toBe(true)
  })
})
