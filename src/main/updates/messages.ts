...
const it = {
  // transport
  accessPrefix: (status: number, detail: string) => `GitHub ha rifiutato l’accesso alle release (HTTP ${status}). ${detail}`,
  rateLimit: (status: number) => `GitHub ha limitato le richieste (HTTP ${status}). Attendi prima di verificare nuovamente.`,
  accessCredentials: 'La sessione GitHub dell’app non è valida o è scaduta. Premi “Collega GitHub e controlla” per accedere nuovamente.',
  ...
  accessSso: 'GitHub richiede l’autorizzazione SSO della credenziale per l’organizzazione. Autorizzala nelle impostazioni GitHub e ripeti il controllo.',
  accessOauthPolicy: 'L’organizzazione limita l’accesso delle applicazioni OAuth. Richiedi l’approvazione di Git Credential Manager per l’account collegato dall’app.',
  accessPermissions: 'La sessione collegata non dispone dei permessi richiesti. Verifica che l’account sia autorizzato a leggere il repository degli aggiornamenti.',
  accessForbidden: 'La richiesta è stata negata. Verifica i permessi sul repository e le eventuali restrizioni dell’organizzazione per l’account o il token usato.',
  httpFailed: (status: number) => `GitHub non ha completato la richiesta di aggiornamento (HTTP ${status}).`,
  ...
  busyGame: 'Un turno della partita è in corso: attendi la mossa dell’avversario prima di installare l’aggiornamento.',
  ...
  genericFailure: 'Operazione di aggiornamento non completata. Riprova dopo aver verificato rete e spazio disponibile.',
  authRequired: 'Premi “Collega GitHub e controlla” e accedi con l’account che vede il repository degli aggiornamenti.',
  authRequiredUnreadable: 'La sessione GitHub salvata dall’app non è leggibile. Premi “Collega GitHub e controlla” per accedere nuovamente.',
  linkedAccount: (account: string) => ` Account collegato dall’app: ${account}.`,
  authInProgress: 'Collegamento GitHub in corso. Nel browser scegli l’account che vede il repository e, se richiesto, completa login e autorizzazione.',
  ...
  versionUnstable: 'La versione corrente non ha una base SemVer stabile: usa un checkout con tag di release o una release ufficiale.',
  ...
  upToDateDev: (base: string, latest: string) => `Versione di sviluppo con base ${base}. Ultima release stabile disponibile: ${latest}.`,
  ...
  suffixPortable: ' Per la versione portable usa la pagina della release e sostituisci manualmente l’eseguibile.',
  ...
  checksumNotUnique: 'Il manifest SHA256SUMS non contiene un checksum univoco per questo installer.',
  checksumConflict: 'I checksum pubblicati da GitHub e SHA256SUMS non coincidono.',
  integrityMismatch: 'Checksum SHA-256 non corrispondente: installer eliminato, installazione annullata.',
  notWindowsExecutable: 'Il download non è un eseguibile Windows valido.',
  downloadedVerified: 'Installer scaricato e SHA-256 verificato. Installa quando sei pronto a riavviare l’app.',
  downloadedUnverified: 'Installer scaricato via HTTPS. La release non pubblica checksum: integrità confrontabile solo con il file locale scaricato.',
  ...
  integrityChangedLocally: 'L’installer locale è cambiato: installazione annullata. Scaricalo nuovamente.',
  finalVerification: 'Verifica finale dell’installer…',
  installerNotStarted: 'Windows non ha avviato l’installer. Verifica autorizzazioni e protezione del sistema.',
  installStarted: 'Aggiornamento avviato. L’app si chiude e viene riaperta dall’installer al termine; partite e impostazioni vengono conservate.',
  ...
  authStorageUnavailable: 'La protezione della sessione GitHub non è disponibile su questo PC. Il collegamento non può essere salvato.',
  gcmMissing: 'Git Credential Manager non disponibile. Installa Git for Windows con Git Credential Manager e riprova.',
  authTimeout: 'Il collegamento GitHub non è terminato entro tre minuti. Ripeti il collegamento e completa login e autorizzazione nel browser.',
  authSpawnFailed: 'Windows non ha avviato Git Credential Manager. Verifica l’installazione di Git for Windows.',
  ...
  authCredentialInvalid: (detail: string) => `Git Credential Manager ha concluso il comando senza restituire una sessione valida. ${detail}`,
  ...
  authSaveFailed: 'Accesso GitHub completato, ma non è stato possibile salvare la sessione cifrata dell’app. Verifica che la cartella dati dell’app sia scrivibile.'
}

type UpdateMessages = typeof it

const en: UpdateMessages = {
  accessPrefix: (status, detail) => `GitHub refused access to the releases (HTTP ${status}). ${detail}`,
  rateLimit: (status) => `GitHub rate-limited the request (HTTP ${status}). Wait before checking again.`,
  accessCredentials: 'The app’s GitHub session is invalid or expired. Press “Connect GitHub and check” to sign in again.',
  ...
  accessSso: 'GitHub requires SSO authorisation of the credential for the organisation. Authorise it in your GitHub settings and check again.',
  accessOauthPolicy: 'The organisation restricts OAuth application access. Request approval of Git Credential Manager for the account connected by the app.',
  accessPermissions: 'The connected session lacks the required permissions. Make sure the account may read the updates repository.',
  accessForbidden: 'The request was denied. Check the repository permissions and any organisation restrictions for the account or token in use.',
  ...
  busyGame: 'A game turn is in progress: wait for the opponent’s move before installing the update.',
  ...
  genericFailure: 'The update operation did not complete. Try again after checking the network and available disk space.',
  authRequired: 'Press “Connect GitHub and check” and sign in with the account that can see the updates repository.',
  authRequiredUnreadable: 'The GitHub session saved by the app cannot be read. Press “Connect GitHub and check” to sign in again.',
  linkedAccount: (account) => ` Account connected by the app: ${account}.`,
  authInProgress: 'Connecting to GitHub. In the browser pick the account that can see the repository and, if asked, complete sign-in and authorisation.',
  ...
  versionUnstable: 'The current version has no stable SemVer base: use a checkout on a release tag or an official release.',
  ...
  upToDateDev: (base, latest) => `Development build based on ${base}. Latest stable release available: ${latest}.`,
  ...
  suffixPortable: ' For the portable build use the release page and replace the executable manually.',
  ...
  integrityMismatch: 'SHA-256 checksum mismatch: the installer was deleted and the installation cancelled.',
  notWindowsExecutable: 'The download is not a valid Windows executable.',
  downloadedVerified: 'Installer downloaded and SHA-256 verified. Install when you are ready to restart the app.',
  downloadedUnverified: 'Installer downloaded over HTTPS. The release publishes no checksum: integrity can only be compared with the downloaded local file.',
  ...
  integrityChangedLocally: 'The local installer changed: installation cancelled. Download it again.',
  finalVerification: 'Final verification of the installer…',
  installerNotStarted: 'Windows did not start the installer. Check permissions and system protection.',
  installStarted: 'Update started. The app closes and is reopened by the installer when it finishes; games and settings are preserved.',
  ...
  authStorageUnavailable: 'Protection of the GitHub session is unavailable on this PC. The connection cannot be saved.',
  gcmMissing: 'Git Credential Manager is unavailable. Install Git for Windows with Git Credential Manager and try again.',
  authTimeout: 'The GitHub connection did not finish within three minutes. Connect again and complete sign-in and authorisation in the browser.',
  authSpawnFailed: 'Windows did not start Git Credential Manager. Check the Git for Windows installation.',
  ...
  authCredentialInvalid: (detail) => `Git Credential Manager ended the command without returning a valid session. ${detail}`,
  ...
  authSaveFailed: 'GitHub sign-in completed, but the app’s encrypted session could not be saved. Check that the app data folder is writable.'
}
