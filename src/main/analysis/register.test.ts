...
class FakeCodex implements SessionCodex {
  ...
  async runTurn(req: TurnRequest, onDelta?: (kind: 'text' | 'reasoning', delta: string) => void): Promise<TurnResult> {
    ...
    const text = req.outputSchema ? JSON.stringify({ takeaways: ['uno', 'due', 'tre', 'quattro'], summary: 'riepilogo finto' }) : 'Commento finto in revisione.'
    return { ok: true, text, turnId: `t-${this.requests.length}`, effectiveModel: null, durationMs: 4 }
  }
...
function fakeEngine(available = true, scoreCp = 20): SessionEngine & { calls: { fen: string; profile: AnalysisProfile }[] } {
  const calls: { fen: string; profile: AnalysisProfile }[] = []
  return {
    calls,
    state: (): EngineState => ({ available, binary: available ? 'avx2' : 'none', version: 'fake 17', message: available ? null : 'no binary' }),
    analyze: async (fen: string, profile: AnalysisProfile): Promise<Analysis> => {
      calls.push({ fen, profile })
      const best = legalMoves(fen)[0]?.uci ?? ''
      return { bestMove: best, lines: [{ move: best, pv: [best], scoreCp, depth: 20 }], depth: 20, fen }
    }
  } as SessionEngine & { calls: { fen: string; profile: AnalysisProfile }[] }
}

function gameOf(sans: string[], patch: Partial<Game> = {}): Omit<Game, 'id' | 'createdAt' | 'updatedAt' | 'status'> {
  const chess = new Chess()
  const moves: Move[] = sans.map((san, index) => {
    const played = chess.move(san)
    return { ply: index + 1, san: played.san, uci: played.lan, fenAfter: chess.fen(), epdAfter: epdOf(chess.fen()), by: index % 2 === 0 ? 'user' : 'ai' }
  })
  return {
    kind: 'match',
    userColor: 'w',
...
describe('AnalysisManager', () => {
  ...
  const saved = async (sans: string[], patch: Partial<Game> = {}): Promise<Game> => {
    ...
    Object.assign(game, { moves: init.moves, result: init.result, analysis: patch.analysis, opening: patch.opening })
  ...
  it('comments a past ply on a `training` thread and saves the text on the move', async () => {
    ...
    expect(codex.started[0]).toMatchObject({ role: 'training', model: 'gpt-6-astra', gameId: game.id })
  ...
  it('streams the whole answer when the turn produced no delta at all', async () => {
    ...
    expect(stream[0]!.payload).toMatchObject({ kind: 'text', chunk: 'Commento finto in revisione.' })
  })
  ...
  it('comments the key moments, one call each, capped at eight', async () => {
    ...
    const game = await saved(sans, {
      analysis: { accuracy: { w: 70, b: 70 }, acpl: { w: 40, b: 40 }, keyMoments, analyzedAt: '2026-03-03T00:00:00.000Z' }
    })
  ...
  it('refuses a lesson that did not come back as the requested JSON', async () => {
    const game = await saved(['e4'])
    codex.script = [{ ok: true, text: 'non è JSON', turnId: 't1', effectiveModel: null, durationMs: 1 }]
  ...
  it('turns a failed review turn into a typed error', async () => {
    ...
    await expect(manager.commentMove(game.id, 1)).rejects.toMatchObject({ code: 'REVIEW_TURN_FAILED' })
  })
