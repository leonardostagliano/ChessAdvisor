...
export function pinnedShortcutRepair(shortcut: ShortcutDetails, executable: string, tempDirectory: string): ShortcutDetails | null {
  if (win32.basename(executable) !== WINDOWS_EXECUTABLE_NAME || win32.basename(shortcut.target).toLowerCase() !== WINDOWS_EXECUTABLE_NAME.toLowerCase()) return null
  if (!samePath(shortcut.target, executable)) {
    ...
    const identified = shortcut.appUserModelId === WINDOWS_APP_ID || samePath(shortcut.icon ?? '', executable)
    if (!identified || !/^ns[a-z0-9]+\.tmp\\old-install\\[^\\]+$/i.test(relative)) return null
  }
  const cwd = win32.dirname(executable)
  if (samePath(shortcut.target, executable) && shortcut.appUserModelId === WINDOWS_APP_ID && samePath(shortcut.cwd ?? '', cwd) && samePath(shortcut.icon ?? '', executable) && shortcut.iconIndex === 0) return null
...
export async function repairPinnedShortcuts(): Promise<void> {
  if (!app.isPackaged || process.platform !== 'win32' || process.env.PORTABLE_EXECUTABLE_DIR || process.env.PORTABLE_EXECUTABLE_FILE) return
  const executable = app.getPath('exe')
  if (win32.basename(executable) !== WINDOWS_EXECUTABLE_NAME) return
  const directory = join(app.getPath('appData'), 'Microsoft', 'Internet Explorer', 'Quick Launch', 'User Pinned', 'TaskBar')
  ...
  for (const entry of entries.slice(0, 200)) {
    ...
    try {
      ...
      const repair = pinnedShortcutRepair(shell.readShortcutLink(path), executable, app.getPath('temp'))
