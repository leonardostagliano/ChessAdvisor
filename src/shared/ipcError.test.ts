...
const overIpc = (name: string, message: string): Error =>
  new Error(`Error invoking remote method 'game:resume': ${name}: ${message}`)

describe('encodeIpcErrorMessage', () => {
  it('writes `code: message` when there is nothing structured to carry', () => {
    expect(encodeIpcErrorMessage('GAME_NOT_FOUND', 'no game with id 42')).toBe(
      'GAME_NOT_FOUND: no game with id 42'
    )
    expect(encodeIpcErrorMessage('GAME_NOT_FOUND', 'no game with id 42', {})).toBe(
      'GAME_NOT_FOUND: no game with id 42'
    )
  ...
  it('appends the payload behind the sentinel', () => {
    const message = encodeIpcErrorMessage('MODEL_UNAVAILABLE', 'the model ghost-1 is gone', {
...
describe('parseIpcError', () => {
  it('reads code, message and payload back through the Electron wrapper', () => {
    const encoded = encodeIpcErrorMessage('MODEL_UNAVAILABLE', 'the model ghost-1 is gone', {
      suggested: 'gpt-6-astra'
    })
  ...
  it('keeps the message intact when there is no payload', () => {
    expect(
      parseIpcError(
        overIpc('IpcError', encodeIpcErrorMessage('AI_THINKING', 'the opponent is still thinking'))
      )
    ).toEqual({
      code: 'AI_THINKING',
  ...
  it('falls back to E_UNEXPECTED on anything it cannot read', () => {
    expect(parseIpcError(new Error('something broke'))).toEqual({
      code: 'E_UNEXPECTED',
  ...
  it('ignores a truncated or corrupt payload instead of throwing', () => {
    const broken = new Error(
      `MODEL_UNAVAILABLE: the model ghost-1 is gone [[ipcdata]]{"suggested":`
    )
    expect(parseIpcError(broken)).toEqual({
      code: 'MODEL_UNAVAILABLE',
      message: 'the model ghost-1 is gone',
      data: {}
    })
...
describe('errorData', () => {
  it('accepts only a plain object', () => {
    expect(errorData(Object.assign(new Error('x'), { data: { suggested: 'a' } }))).toEqual({
      suggested: 'a'
    })
