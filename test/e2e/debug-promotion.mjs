...
const until = async (fn, timeout, label) => { const t = Date.now() + timeout; for (;;) { let v = null; try { v = await fn() } catch {} if (v) return v; if (Date.now() > t) throw new Error('timeout ' + label); await sleep(200) } }
const state = () => page.evaluate(() => window.api.game.state())
try {
  await until(async () => (await page.evaluate(() => window.api.codex.state())).status === 'ready', 120000, 'codex')
  const s = await page.evaluate(() => window.api.game.new({ userColor: 'w', model: 'gpt-6-astra', effort: 'medium', difficulty: { mode: 'fixed', level: 6 }, coach: { model: 'gpt-6-astra', effort: 'medium' }, language: 'it', showReasoning: false, commentsVisible: false, startFen: '8/4P3/8/8/8/8/8/K6k w - - 0 1', kind: 'endgame_drill' }))
  console.log('game:', s.status, s.fen)
  await page.locator('cg-board').first().waitFor({ timeout: 30000 }); await sleep(500)
  const board = page.locator('cg-board').first(); const b = await board.boundingBox(); const size = b.width / 8
  const c = (sq) => ({ x: b.x + (sq.charCodeAt(0) - 97) * size + size / 2, y: b.y + (7 - (Number(sq[1]) - 1)) * size + size / 2 })
  const a = c('e7'), z = c('e8')
  ...
  console.log('picker visible:', picker === 1, '| moves before choice:', (await state()).game.moves.length)
  ...
  await until(async () => { const st = await state(); return st.userToMove || st.status !== 'playing' }, 30000, 'ai reply')
  if ((await state()).status === 'playing') {
    ...
    await sleep(400)
    const shapes = await page.evaluate(() => document.querySelectorAll('cg-board ~ svg g > *, .cg-shapes g > *').length)
...
} catch (e) { console.log('EXC', String(e).slice(0, 400)) } finally {
